...
def visualize(public_info):
    ...
    for i in range(coordinates.shape[0]):
        ...
        ax.text(x=coordinates[i][0], y=coordinates[i][1], s=plc_txt, fontsize=10, ha='center', va='center',
                bbox=dict(boxstyle='round,pad=0.1', fc=color_list[i], ec='w', lw=1, alpha=0.5))

    # Add player info
    for ply in ply_info:
        ply['Inventory'] = {output_null(item): ply['Inventory'].count(item) for item in ['Bean', 'Corn', 'Pig', 'Cow']
        ...
        if ply['Position'] == 0:
            ...
            color = ['r', 'y', 'g', 'b'][idx]
        ax.text(x=-4, y=1 - idx, s=ply_txt, fontsize=14, ha='left', va='center',
