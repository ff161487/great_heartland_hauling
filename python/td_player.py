from td_choice import all_choices
from td_parameters import INIT_MONEY, HAND_LIMIT, NOT_SPEC_COST
...
class Player:
    ...
    def refuel(self, draw_pile):
        if len(self.hand) < HAND_LIMIT:
            draw_pile = self.draw(draw_pile, HAND_LIMIT - len(self.hand))
        return draw_pile
    ...
    def take_action(self, place, draw_pile, discard_pile, action_array):
        ...
        discard_pile = self.discard(discard_pile, tbd)
        if action_array[3] == 1:
